...
def send_message():
    ...
    if file and file.filename and allowed_file(file.filename):
        ...
        try:
            ...
            response_content = """Hier ist ein einfaches "Hallo, Welt!"-Programm in Python:
```python
# Hallo_Welt.py

def main():
    # Ausgabe des Grusswortes
    print("Hallo, Welt!")

if __name__ == "__main__":
    main()
```
Lassen Sie mich durch das Beispiel gehen:

* Wir definieren eine Funktion `main()`, die das Hauptprogramm enthält.
* Innerhalb der Funktion wird mit `print()` das Grüsswort "Hallo, Welt!" auf die Konsole ausgegeben.
* Die Zeile `if __name__ == "__main__":` ist ein Standard-Python-Konstrukt, das sicherstellt, dass die Funktion
`main()` nur dann ausgeführt wird, wenn der Skript direkt aufgerufen wird (und nicht, wenn es von einem anderen
Skript eingebunden wird).

Um dieses Programm auszuführen, können Sie es in einem Texteditor wie Notepad++ oder Sublime Text speichern und
als Python-Datei (`Hallo_Welt.py`) über die Kommandozeile mit `python Hallo_Welt.py` starten.

Wenn Sie ein IDE (Integrated Development Environment) wie PyCharm verwenden, können Sie das Programm direkt
innerhalb des Editors ausführen."""
