...
def ensure_directory(path: str):
    """
    Ensures that a directory exists. If not, it will be created.
    """
    try:
        os.makedirs(path, exist_ok=True)
        logging.debug(f"Directory ensured: {path}")
    except Exception as e:
        logging.error(f"Error while creating directory {path}: {str(e)}")
        raise


def get_project_paths_cpp():
    """
    Initializes the C++ project directory and necessary file paths without relying on external scripts.
    """
    base_dir = os.path.expanduser("~/Projects/MAVIS")
    cpp_dir = os.path.join(base_dir, "mavis-cpp-compiler")
    ready_indicator = os.path.join(cpp_dir, "cpp_compiler_ready.txt")

    ensure_directory(base_dir)
    ensure_directory(cpp_dir)

    # Create file to indicate that the setup is ready
    if not os.path.exists(ready_indicator):
        with open(ready_indicator, "w", encoding="utf-8") as f:
            f.write("ready")
            logging.info(f"Ready indicator created: {ready_indicator}")
    else:
        logging.debug(f"Ready indicator already exists: {ready_indicator}")

    return base_dir, cpp_dir, ready_indicator


def compile_cpp_with_gpp(cpp_filename, exe_filename):
    """
    Compiles the specified C++ file using g++ on Linux.
    Returns (success: bool, error_message: str).
    """
    logging.info(f"Compile {cpp_filename} with g++...")
    command = ["g++", "-std=c++17", cpp_filename, "-o", exe_filename]
...
def execute_cpp_code(md_content: str) -> str:
    """
    Finds C++ code blocks in Markdown, compiles them with g++, executes them and returns the output as HTML.
    """
    ...
    for idx, code in enumerate(matches, start=1):
        ...
        exe_filename = os.path.join(tmp_dir, f"cpp_block_{timestamp}_{idx}")
        with open(cpp_filename, "w", encoding="utf-8") as f:
            f.write(code)

        success, compile_err = compile_cpp_with_gpp(cpp_filename, exe_filename)
        if not success:
...
def get_project_paths_c():
    """
    Initializes the C project directory and necessary file paths without external .sh scripts.
    """
    base_dir = os.path.expanduser("~/Projects/MAVIS")
    c_dir = os.path.join(base_dir, "mavis-c-compiler")
    ready_indicator = os.path.join(c_dir, "c_compiler_ready.txt")

    ensure_directory(base_dir)
    ensure_directory(c_dir)

    # Create file as compiler readiness marker
    if not os.path.exists(ready_indicator):
        with open(ready_indicator, "w", encoding="utf-8") as f:
            f.write("ready")
            logging.info(f"Ready indicator created: {ready_indicator}")
    ...
    return base_dir, c_dir, ready_indicator


def compile_c_with_gcc(c_filename, exe_filename):
    """
    Compiles the specified C file using gcc on Linux.
    Returns (success: bool, error_message: str).
    """
    logging.info(f"Compile {c_filename} with gcc...")
    command = ["gcc", "-std=c11", c_filename, "-o", exe_filename]
...
def execute_c_code(md_content: str) -> str:
    """
    Finds C code blocks in Markdown, compiles them with gcc, executes them and returns the output as HTML.
    """
    ...
    for idx, code in enumerate(matches, start=1):
        ...
        exe_filename = os.path.join(tmp_dir, f"c_block_{timestamp}_{idx}")
        with open(c_filename, "w", encoding="utf-8") as f:
            f.write(code)

        success, compile_err = compile_c_with_gcc(c_filename, exe_filename)
        if not success:
            outputs.append(f"Compilation failed:\n{compile_err}")
            continue
...
def send_message():
    ...
    if file and file.filename and allowed_text_file(file.filename):
        ...
        try:
            ...
            if python_output:
                response_content_code += f"<div><pre><p><strong>Python</strong>: {python_output}</p></pre></div>"
            if cpp_output:
                response_content_code += f"<div><pre><p><strong>C++</strong>: {cpp_output}</p></pre></div>"
            if c_output:
