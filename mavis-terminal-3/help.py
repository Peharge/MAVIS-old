...
def show_help():
    ...
    print(
        f" - To {blue}run Python scripts{blue} in PowerShell, navigate to the script directory and execute the command:")
    ...
    print(
        " - If you're using PowerShell as an administrator and face any permission issues, try running it as Administrator.\n")
    ...
    print(" - After installation, you can use the following commands to interact with Ollama:\n")
    ...
    print("  Example: ollama --version")

    print(" - For additional help on any command, type:")
    print(f"  {blue}ollama [command] --help{reset}")
    print("  Example: ollama run --help")
    ...
    print(" - To navigate between directories, use the {blue}cd{reset} command:")
    print(f"  Example: {blue}cd C:\\Users\\UserName\\Documents{reset} - Changes the directory to Documents.")
    print(" - To go back to the previous directory, use {blue}cd ..{reset}.")
    print(" - To go to the root of the current drive, use {blue}cd \\{reset}.")
    print(" - To create a new directory, use {blue}mkdir{reset}:")
    print(f"  Example: {blue}mkdir NewFolder{reset} - Creates a new folder called 'NewFolder'.")
    print(" - To remove a file, use {blue}rm{reset}, and to remove an empty directory, use {blue}rmdir{reset}:")
    ...
    print(" - To view the contents of a file, use the {blue}cat{reset} command:")
    print(f"  Example: {blue}cat file.txt{reset} - Displays the contents of 'file.txt'.")
    print(" - To copy files or directories, use {blue}cp{reset}:")
    print(f"  Example: {blue}cp file.txt D:\\Backup{reset} - Copies 'file.txt' to the 'D:\\Backup' directory.")
    print(" - To move or rename files, use {blue}mv{reset}:")
    print(f"  Example: {blue}mv file.txt newfile.txt{reset} - Renames 'file.txt' to 'newfile.txt'.")
    print(" - To find files or folders, use the {blue}find{reset} command:")
    print(f"  Example: {blue}find . -name 'file.txt'{reset} - Searches for 'file.txt' in the current directory and subdirectories.")
    print(" - To search inside a file, use the {blue}select-string{reset} command:")
    print(f"  Example: {blue}select-string -Path 'file.txt' -Pattern 'keyword'{reset} - Searches for 'keyword' in 'file.txt'.")
    print(" - To get a list of running processes, use {blue}Get-Process{reset}:")
    print(f"  Example: {blue}Get-Process{reset} - Lists all active processes.")
    print(" - To stop a process, use {blue}Stop-Process{reset}:")
    print(f"  Example: {blue}Stop-Process -Name notepad{reset} - Stops the Notepad process.")
    print(" - To view system information, use {blue}Get-ComputerInfo{reset} or {blue}systeminfo{reset}.")
    print(" - To get help on a specific command, use {blue}Get-Help <command>{reset}:")
    print(f"  Example: {blue}Get-Help dir{reset} - Displays help for the 'dir' command.")
    print(" - To view the command history, use {blue}Get-History{reset}.")
    print(" - To clear the screen, use {blue}Clear{reset}.")
    print(" - To get the current working directory, use {blue}Get-Location{reset}.")
    print(" - To set the current location (directory), use {blue}Set-Location{reset}:")
    print(f"  Example: {blue}Set-Location C:\\Users\\UserName{reset} - Changes the current directory to 'C:\\Users\\UserName'.")
    print(" - To run a script, make sure the execution policy allows it. To set the execution policy, use:")
    print(f"  {blue}Set-ExecutionPolicy RemoteSigned{reset} - Allows running scripts locally and downloaded ones if signed.")
    print(" - To run a script, navigate to its location and type the script name with a .ps1 extension:")
    print(f"  Example: {blue}.\\script.ps1{reset}")
    print(" - To see all the environment variables, use {blue}Get-ChildItem Env:{reset}.")
    print(" - To add a new environment variable, use {blue}$env:VariableName = 'Value'{reset}:")
    print(f"  Example: {blue}$env:MyVar = 'Hello'{reset} - Sets the environment variable 'MyVar' to 'Hello'.")
    print(" - To display the contents of an environment variable, use {blue}$env:VariableName{reset}:")
    print(f"  Example: {blue}$env:MyVar{reset} - Displays the value of 'MyVar'.")
    print(" - To change the default drive, use the {blue}cd{reset} command followed by the drive letter:")
    print(f"  Example: {blue}cd D:\\{reset} - Switches to the D: drive.")
    print(" - To get the current date and time, use {blue}Get-Date{reset}.")
    print(f"  Example: {blue}Get-Date{reset} - Displays the current date and time.")
    print(" - To format the date and time output, use {blue}Get-Date -Format <format>{reset}:")
    print(f"  Example: {blue}Get-Date -Format 'yyyy-MM-dd HH:mm:ss'{reset} - Displays the date in the specified format.")
    print(" - For advanced scripting, you can write scripts in .ps1 files, which are executed by typing:")
    print(f"  {blue}.\\script_name.ps1{reset} - Executes the 'script_name.ps1' script.")
    print(" - To make sure scripts can run, set the execution policy with {blue}Set-ExecutionPolicy RemoteSigned{reset}.")
    print(" - To use pipelines in PowerShell (sending the output of one command to another), use {blue}|{reset}:")
    ...
    print(" - To create a loop, use {blue}for, foreach, while, do{reset} loops. Example:")
    print(f"  {blue}foreach ($item in 1..5) {{Write-Output $item}}{reset} - Prints numbers 1 to 5.")
    print(" - You can use {blue}if{reset} statements for conditional execution:")
    print(f"  Example: {blue}if ($a -gt $b) {{Write-Output 'a is greater than b'}}{reset}")
    print(" - PowerShell supports object-oriented scripting, allowing you to work with .NET objects, classes, and methods.")
