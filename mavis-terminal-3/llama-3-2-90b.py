...
def encode_image_to_base64(image_path):
    """Kodiert das Bild in Base64 für den Upload."""
    try:
        with open(image_path, "rb") as image_file:
            return base64.b64encode(image_file.read()).decode('utf-8')
    except Exception as e:
        print(f"{red}Error encoding image: {e}{reset}")
        return None


def get_response_from_ollama(user_message, image_path=None, ollama=None):
    """Fragt Ollama nach einer Antwort auf die Benutzereingabe und übermittelt ein Bild."""
    try:
        images = []
        if image_path:
            base64_image = encode_image_to_base64(image_path)
            if base64_image:
                images = [{'base64': base64_image}]

        response = ollama.chat(
            model="llama3.2-vision:90b",  # Modellname
            messages=[{"role": "user", "content": user_message}],
            images=images  # Hier wird das Bild übergeben
        )
...
def main():
    ...
    if check_command_installed("ollama"):
        ...
        models_to_check = ["llama3.2-vision:90b"]
        ...
        while True:
            ...
            response = get_response_from_ollama(user_input, image_path=image_path, ollama=ollama)
