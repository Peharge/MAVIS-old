...
venv_path = "/home/julia/PycharmProjects/MAVIS/.env"
...
{blue}MAVIS License{reset}: MIT
    ...
    # Function to display the 16 color palette without spaces and numbers
    def show_color_palette():
        for i in range(8):
            print(f"\033[48;5;{i}m  \033[0m", end="")  # Colors without numbers and without spaces

        print()  # Line break after the first row

        # Display the lighter colors (8-15) without spaces and numbers
        for i in range(8, 16):
            print(f"\033[48;5;{i}m  \033[0m", end="")

        print()  # Another line break at the end

    # Call the function to show the color palette
    show_color_palette()
...
def run_command(command, shell=False):
    ...
    # Ensure pip commands use the correct Python path
    if 'pip' in command:
    ...
    while process.poll() is None or stdout_lines or stderr_lines:
        ...
        time.sleep(1 / 24)  # 24 FPS update

    # Ensure the last line
    while stdout_lines:
        print(stdout_lines.pop(0), end='', flush=True)
    while stderr_lines:
        print(stderr_lines.pop(0), end='', flush=True, file=sys.stderr)
...
def handle_special_commands(user_input):
    # Load the .env file
    load_dotenv(dotenv_path="/home/julia/PycharmProjects/MAVIS/.env")

    # The path to the Python interpreter in the .env
    python_path = "/home/julia/PycharmProjects/MAVIS/.env/bin/python"
    ...
    if user_input in commands:
        ...
        # Check if it is a Python file or a Shell script
        if not user_input.endswith(".sh"):
            # Execute the script with the Python interpreter from the .env environment
            run([python_path, script_path], shell=True)
        else:
            # If it is a Shell script, execute it directly
...
def main():
    ...
    while True:
        try:
            ...
            # Display the input prompt before the user types anything
            ...
            # Ensure all outputs are completed before the next input prompt appears
            sys.stdout.flush()
            sys.stderr.flush()
