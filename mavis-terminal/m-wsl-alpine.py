...
"""
Comprehensive Alpine System Information in WSL – Enhanced Design

This enhanced PyQt6 application displays detailed system information obtained from Alpine running in WSL.
...
def detect_alpine_distro():
    """
    Detects installed WSL distributions and returns the first Alpine-based one found.
    """
    try:
        output = subprocess.check_output(["wsl", "--list", "--quiet"], text=True)
        for line in output.splitlines():
            if "Alpine" in line:
                logging.info(f"Detected Alpine distro: {line.strip()}")
                return line.strip()
        raise RuntimeError("No Alpine WSL distribution found.")
    except Exception as e:
        logging.error(f"Could not detect Alpine WSL distribution: {e}")
        return "Alpine"  # Fallback
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("MAVIS Alpine System Information in WSL – Enhanced Design")
    ...
    def initUI(self):
        ...
        self.title_label = QLabel("Alpine System Overview")
