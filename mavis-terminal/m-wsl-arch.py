...
"""
Comprehensive Arch System Information in WSL – Enhanced Design

This enhanced PyQt6 application displays detailed system information obtained from Arch running in WSL.
...
def detect_arch_distro():
    """
    Detects installed WSL distributions and returns the first Arch-based one found.
    """
    try:
        output = subprocess.check_output(["wsl", "--list", "--quiet"], text=True)
        for line in output.splitlines():
            if "Arch" in line:
                logging.info(f"Detected Arch distro: {line.strip()}")
                return line.strip()
        raise RuntimeError("No Arch WSL distribution found.")
    except Exception as e:
        logging.error(f"Could not detect Arch WSL distribution: {e}")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("MAVIS Arch System Information in WSL – Enhanced Design")
    ...
    def initUI(self):
        ...
        self.title_label = QLabel("Arch System Overview")
