...
"""
Comprehensive ClearLinux System Information in WSL – Enhanced Design

This enhanced PyQt6 application displays detailed system information obtained from ClearLinux running in WSL.
...
def detect_clearlinux_distro():
    """
    Detects installed WSL distributions and returns the first ClearLinux-based one found.
    """
    try:
        output = subprocess.check_output(["wsl", "--list", "--quiet"], text=True)
        for line in output.splitlines():
            if "ClearLinux" in line:
                logging.info(f"Detected ClearLinux distro: {line.strip()}")
                return line.strip()
        raise RuntimeError("No ClearLinux WSL distribution found.")
    except Exception as e:
        logging.error(f"Could not detect ClearLinux WSL distribution: {e}")
        return "ClearLinux"  # Fallback
...
class SystemInfoWorker(QThread):
    ...
    def run(self):
        ...
        distro = detect_clearlinux_distro()
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("MAVIS ClearLinux System Information in WSL – Enhanced Design")
    ...
    def initUI(self):
        ...
        self.title_label = QLabel("ClearLinux System Overview")
