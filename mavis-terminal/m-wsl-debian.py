...
"""
Comprehensive Debian System Information in WSL – Enhanced Design

This enhanced PyQt6 application displays detailed system information obtained from Debian running in WSL.
...
def detect_debian_distro():
    """
    Detects installed WSL distributions and returns the first Debian one found.
    """
    try:
        output = subprocess.check_output(["wsl", "--list", "--quiet"], text=True)
        for line in output.splitlines():
            if "Debian" in line:
                logging.info(f"Detected Debian distro: {line.strip()}")
                return line.strip()
        raise RuntimeError("No Debian WSL distribution found.")
    except Exception as e:
        logging.error(f"Could not detect Debian WSL distribution: {e}")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("MAVIS Debian System Information in WSL – Enhanced Design")
    ...
    def initUI(self):
        ...
        self.title_label = QLabel("Debian System Overview")
