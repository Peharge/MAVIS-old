...
"""
Comprehensive Kali System Information in WSL – Enhanced Design

This enhanced PyQt6 application displays detailed system information obtained from Kali running in WSL.
...
def detect_kali_distro():
    """
    Detects installed WSL distributions and returns the first kali-based one found.
    """
    try:
        output = subprocess.check_output(["wsl", "--list", "--quiet"], text=True)
        for line in output.splitlines():
            if "kali-linux" in line:
                logging.info(f"Detected Kali distro: {line.strip()}")
                return line.strip()
        raise RuntimeError("No Kali WSL distribution found.")
    except Exception as e:
        logging.error(f"Could not detect Kali WSL distribution: {e}")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("MAVIS Kali System Information in WSL – Enhanced Design")
    ...
    def initUI(self):
        ...
        self.title_label = QLabel("Kali System Overview")
