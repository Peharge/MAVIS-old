...
"""
Comprehensive Mint System Information in WSL – Enhanced Design

This enhanced PyQt6 application displays detailed system information obtained from Mint running in WSL.
...
def detect_mint_distro():
    """
    Detects installed WSL distributions and returns the first mint-based one found.
    """
    try:
        output = subprocess.check_output(["wsl", "--list", "--quiet"], text=True)
        for line in output.splitlines():
            if "mint" in line:
                logging.info(f"Detected mint distro: {line.strip()}")
                return line.strip()
        raise RuntimeError("No mint WSL distribution found.")
    except Exception as e:
        logging.error(f"Could not detect mint WSL distribution: {e}")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("MAVIS mint System Information in WSL – Enhanced Design")
    ...
    def initUI(self):
        ...
        self.title_label = QLabel("mint System Overview")
