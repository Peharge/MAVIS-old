...
"""
Comprehensive openSUSE System Information in WSL – Enhanced Design

This enhanced PyQt6 application displays detailed system information obtained from openSUSE running in WSL.
...
def detect_opensuse_distro():
    """
    Detects installed WSL distributions and returns the first opensuse-based one found.
    """
    try:
        output = subprocess.check_output(["wsl", "--list", "--quiet"], text=True)
        for line in output.splitlines():
            if "openSUSE-Leap" in line:
                logging.info(f"Detected opensuse distro: {line.strip()}")
                return line.strip()
        raise RuntimeError("No opensuse WSL distribution found.")
    except Exception as e:
        logging.error(f"Could not detect opensuse WSL distribution: {e}")
        return "openSUSE-Leap"  # Fallback
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("MAVIS opensuse System Information in WSL – Enhanced Design")
    ...
    def initUI(self):
        ...
        self.title_label = QLabel("opensuse System Overview")
