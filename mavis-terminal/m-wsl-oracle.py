...
"""
Comprehensive OracleLinux System Information in WSL – Enhanced Design

This enhanced PyQt6 application displays detailed system information obtained from OracleLinux running in WSL.
...
def detect_oraclelinux_distro():
    """
    Detects installed WSL distributions and returns the first OracleLinux-based one found.
    """
    try:
        output = subprocess.check_output(["wsl", "--list", "--quiet"], text=True)
        for line in output.splitlines():
            if "OracleLinux" in line:
                logging.info(f"Detected OracleLinux distro: {line.strip()}")
                return line.strip()
        raise RuntimeError("No OracleLinux WSL distribution found.")
    except Exception as e:
        logging.error(f"Could not detect OracleLinux WSL distribution: {e}")
        return "OracleLinux"  # Fallback
...
class SystemInfoWorker(QThread):
    ...
    def run(self):
        ...
        distro = detect_oraclelinux_distro()
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("MAVIS OracleLinux System Information in WSL – Enhanced Design")
    ...
    def initUI(self):
        ...
        self.title_label = QLabel("OracleLinux System Overview")
