...
"""
Comprehensive Pengwin System Information in WSL – Enhanced Design

This enhanced PyQt6 application displays detailed system information obtained from Pengwin running in WSL.
...
def detect_pengwin_distro():
    """
    Detects installed WSL distributions and returns the first Pengwin-based one found.
    """
    try:
        output = subprocess.check_output(["wsl", "--list", "--quiet"], text=True)
        for line in output.splitlines():
            if "Pengwin" in line:
                logging.info(f"Detected Pengwin distro: {line.strip()}")
                return line.strip()
        raise RuntimeError("No Pengwin WSL distribution found.")
    except Exception as e:
        logging.error(f"Could not detect Pengwin WSL distribution: {e}")
        return "Pengwin"  # Fallback
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("MAVIS Pengwin System Information in WSL – Enhanced Design")
    ...
    def initUI(self):
        ...
        self.title_label = QLabel("Pengwin System Overview")
