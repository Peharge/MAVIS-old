...
"""
Comprehensive SLES System Information in WSL – Enhanced Design

This enhanced PyQt6 application displays detailed system information obtained from SLES running in WSL.
...
def detect_sles_distro():
    """
    Detects installed WSL distributions and returns the first SLES-based one found.
    """
    try:
        output = subprocess.check_output(["wsl", "--list", "--quiet"], text=True)
        for line in output.splitlines():
            if "SLES" in line:
                logging.info(f"Detected SLES distro: {line.strip()}")
                return line.strip()
        raise RuntimeError("No SLES WSL distribution found.")
    except Exception as e:
        logging.error(f"Could not detect SLES WSL distribution: {e}")
        return "SLES"  # Fallback
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("MAVIS SLES System Information in WSL – Enhanced Design")
    ...
    def initUI(self):
        ...
        self.title_label = QLabel("SLES System Overview")
