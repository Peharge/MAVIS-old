...
"""
Comprehensive Ubuntu System Information in WSL – Enhanced Design

This enhanced PyQt6 application displays detailed system information obtained from Ubuntu running in WSL.
...
def detect_ubuntu_distro():
    """
    Detects installed WSL distributions and returns the first Ubuntu-based one found.
    """
    try:
        output = subprocess.check_output(["wsl", "--list", "--quiet"], text=True)
        for line in output.splitlines():
            if "Ubuntu" in line:
                logging.info(f"Detected Ubuntu distro: {line.strip()}")
                return line.strip()
        raise RuntimeError("No Ubuntu WSL distribution found.")
    except Exception as e:
        logging.error(f"Could not detect Ubuntu WSL distribution: {e}")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("MAVIS Ubuntu System Information in WSL – Enhanced Design")
    ...
    def initUI(self):
        ...
        self.title_label = QLabel("Ubuntu System Overview")
