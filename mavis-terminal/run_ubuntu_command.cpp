...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        std::cerr << "[ERROR] Kein Befehl angegeben." << std::endl;
        return 1;
    }

    // Baue den WSL-Befehl zusammen.
    // Wir verwenden jetzt "wsl.exe -d ubuntu" und hängen dann alle übergebenen Parameter an.
    ...
    std::cout << "Führe folgenden Befehl auf Linux aus: " << fullCommand << std::endl;
    ...
    if (!success) {
        std::cerr << "[ERROR] Prozess konnte nicht gestartet werden. Fehlercode: " << GetLastError() << std::endl;
        return 1;
    }
